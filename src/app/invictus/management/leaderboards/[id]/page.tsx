...
export default function LeaderboardEntriesAdminPage() {
    ...
    const handleRecalculate = async () => {
        const result = await dispatch(
            recalculateLeaderboardRanks(leaderboardId),
        );
    ...
    return (
        <PageContainer variant="invictus">
            ...
            <PageHeader
                ...
                eyebrow={
                    selectedLeaderboard
                        ? `${selectedLeaderboard.type} · ${selectedLeaderboard.period}`
                        : "Leaderboard"
                }
                title={
                    loading
                    ...
                    <div className="flex gap-2">
                        {selectedLeaderboard?.status === "active" && (
                            <Button
                                ...
                                onClick={() =>
                                    dispatch(finalizeLeaderboard(leaderboardId))
                                }
                            >
                                Finalize
                            </Button>
            ...
            {isFinalized && (
                <p className="mt-4 rounded-md bg-gold/5 p-3 text-sm text-gold">
                    This leaderboard is finalized — points and entries can no
                    longer be modified.
                </p>
            )}
