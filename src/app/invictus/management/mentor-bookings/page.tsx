...
export default function AdminMentorBookingsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<MentorBookingStatus | "all">(
    "all",
  );
  ...
  return (
    <PageContainer className="min-h-screen w-full max-w-full overflow-x-hidden bg-[#FBF9F4]">
      <div className="page-wrapper min-w-0 w-full space-y-6">
        {/* Header */}
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[#6B6B6B]">
              Manage, review and coordinate all mentor accountability sessions.
            </p>
          </div>
          ...
          >
            <RefreshCw
              className={`mr-2 h-4 w-4 ${
                status.adminList === "loading" ? "animate-spin" : ""
              }`}
            />
            {status.adminList === "loading" ? "Refreshing..." : "Refresh"}
          </Button>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-5">
          ...
          <StatCard title="Requested" value={stats.requested} icon={Clock} />
          <StatCard
            title="Confirmed"
        ...
        {/* Filters */}
        <Card className="border-[#E9E2D2] rounded-lg bg-white w-full">
          <CardContent className="p-4">
            <div className="flex flex-col gap-3 lg:flex-row">
              ...
              <Select
                value={statusFilter}
                onValueChange={(value) => {
                  setStatusFilter(value as MentorBookingStatus | "all");
                  setPage(1);
                }}
              >
        ...
        <Card className="w-full max-w-full overflow-hidden border border-[#E9E2D2] rounded-lg  overflow-hidden rounded-2xl">
          ...
          <CardContent className="p-0">
            {/* Table internal scroll container */}
            <div className="w-full overflow-x-auto">
              <table className="w-full min-w-[700px] text-left">
                <thead>
                  <tr className="border-b bg-[#FAF6EE] text-[#8A8175] text-xs uppercase tracking-wide">
                    ...
                    filteredBookings.map((booking) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 align-top">
                          <p className="font-medium text-gray-900 break-words">
                            {booking.leadMentor?.fullName || "Unknown mentor"}
                          </p>
                          {booking.coMentor && (
                            <p className="text-xs text-gray-500 break-words">
                              Co: {booking.coMentor.fullName}
                            </p>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <Button
                            ...
                            disabled={status.adminFetchSingle === "loading"}
                          >
      ...
      <Sheet open={detailsOpen} onOpenChange={handleDetailsOpenChange}>
        <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
              ...
              <Button
                ...
                onClick={() => {
                  if (requestedBookingId) {
                    isFetchingRef.current = true;
                    dispatch(
                      fetchSingleMentorBookingAdmin(requestedBookingId),
                    ).finally(() => {
                      isFetchingRef.current = false;
                    });
                  }
          ...
          {status.adminFetchSingle === "succeeded" && selectedBooking && (
            <div className="mt-6">
