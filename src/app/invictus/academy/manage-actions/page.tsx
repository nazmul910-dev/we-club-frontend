    ...
    <div className="page-wrapper">
        ...
        >
          <PopoverTrigger className="block w-full">
            <Button
              type="button"
              variant="outline"
              className="h-auto min-h-12 w-full justify-between rounded-xl border-[#E7DDCC] bg-white p-3 text-left text-sm font-normal"
            >
              <span className={cn(!courseFilter && "text-muted-foreground")}>
                {courseFilter
                  ? courses.find((course) => course._id === courseFilter)?.title
                  : "All Courses"}
              </span>
      ...
      <div className="mt-6">
        {loading ? (
          <TableSkeleton
            variant="invictus"
            className="border-gold-soft border"
          />
        ) : (
          <ActionTable
