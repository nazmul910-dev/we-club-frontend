...
function ManageProgressContent() {
  ...
  const { records, meta, loading, error } = useAppSelector((state) => state.progress);
  ...
  const [selectedRecord, setSelectedRecord] = useState<IModuleProgress | null>(null);
  ...
  useEffect(() => {
    dispatch(
      fetchAllProgress({
        ...
        isCompleted: statusFilter === "" ? undefined : statusFilter === "completed",
        page: 1,
        limit: 50,
      }),
    );
  ...
  const stats = useMemo(() => {
    ...
    const avgCompletion = records.length === 0 ? 0 : Math.round(records.reduce((sum, item) => sum + item.overallCompletionPercent, 0) / records.length);
  ...
  return (
    <div className="mx-auto max-w-[1180px] px-[6vw] py-[2vw] sm:px-8">
      ...
      {error && (
        <div className="mt-5 rounded-xl bg-red-50 p-3 text-sm text-red-500">{error}</div>
      )}

      <div className="mt-8 grid gap-6 md:grid-cols-4">
        <StatCard icon={<ListChecks />} title="Total Records" value={String(stats.total)} />
        <StatCard icon={<CircleCheck />} title="Modules Completed" value={String(stats.completed)} />
        <StatCard icon={<Loader2 />} title="In Progress" value={String(stats.inProgress)} />
        <StatCard icon={<Gauge />} title="Avg. Completion" value={`${stats.avgCompletion}%`} />
      </div>
      ...
      <div className="mt-6">
        {loading ? (
          <p className="text-sm text-[#8A8175]">Loading progress records...</p>
        ) : (
          <ProgressTable
            data={records}
            onView={(record) => {
