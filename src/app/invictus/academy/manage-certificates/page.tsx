...
import { CertificateStatus, IQuizCertificate } from "@/lib/features/invictus/academy/cerfificate/certificateTypes";
...
function ManageCertificatesContent() {
  ...
  const { certificates, meta, loading, error } = useAppSelector((state) => state.certificate);
    ...
    <div className="mx-auto max-w-[1180px] px-[6vw] py-[2vw] sm:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-semibold text-[#171717]">Certificates</h1>
          <p className="mt-2 text-sm text-[#8A8175]">
            Review every certificate earned by passing a module quiz, attach the generated file and revoke when needed.
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-3">
        <StatCard icon={<Award />} title="Total Certificates" value={String(stats.total)} />
        <StatCard icon={<CircleCheck />} title="Active / Issued" value={String(stats.issued)} />
        <StatCard icon={<ShieldAlert />} title="Revoked" value={String(stats.revoked)} />
      </div>
      ...
      <div className="mt-6">
        {loading ? (
          <p className="text-sm text-[#8A8175]">Loading certificates...</p>
        ) : (
          <CertificateTable
            data={certificates}
            onView={(certificate) => {
