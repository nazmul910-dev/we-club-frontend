...
const EditSessionModal = dynamic(
    () => import("@/components/invictus/academy/sessions/EditSessionModal"),
    {
        ssr: false,
    },
);
const AttendeesModal = dynamic(
    () => import("@/components/invictus/academy/sessions/AttendeesModal"),
    {
        ssr: false,
    },
);
...
export default function ManageSessionsPage() {
    return (
        <AuthGuard
            allowedRoles={["founder", "manager", "admin", "super_admin"]}
        >
...
function ManageSessionsContent() {
    ...
    const [editSession, setEditSession] = useState<ISessionScheduleItem | null>(
        null,
    );
    const [attendeesSession, setAttendeesSession] =
    ...
    const confirmCancel = async () => {
        ...
        try {
            ...
            toast.error(
                err?.response?.data?.message || "Could not cancel session",
            );
        } finally {
            setCancelling(false);
        }
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            toast.error(
                err?.response?.data?.message || "Could not delete session",
            );
        } finally {
            setDeleting(false);
        }
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="mt-1.5 text-sm text-[#8A8175]">
                        Create and manage live calls, masterclasses, 1:1
                        sessions, and track registered attendees.
                    </p>
                </div>
            ...
            <div className="mt-6">
                {loading ? (
                    ...
                    <div className="rounded-2xl border border-dashed border-[#E7DDCC] p-12 text-center bg-[#FFFCF7]">
                        <CalendarClock
                            className="mx-auto mb-3 text-[#B08A3E]"
                            size={32}
                        />
                        <p className="text-base font-semibold text-[#1C1A16]">
                            No sessions found
                        </p>
                                ...
                                {filteredSessions.map((s) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4 text-xs text-[#8A8175]">
                                            ...
                                            <span className="text-[10px] text-[#B0A996]">
                                                ({s.timezone})
                                            </span>
                                        </td>
