...
interface Props {
  commission: any;
  type: "confirm" | "paid" | "dispute" | "resolve" | "send" | "receive";
  close: () => void;
}
...
export default function CommissionModal({ commission, type, close }: Props) {
  ...
  const submit = async () => {
    try {
      ...
      if (type === "paid") {
        // Payment method was already recorded when the owner sent the
        // payment — admin just confirms, no method selection needed here.
        const result = await dispatch(
          markPaid({
            id: commission._id,
          }),
      ...
      if (type === "send") {
        ...
        if (sendCommissionPayment.fulfilled.match(result)) {
          close();
        }
        return;
      }
      ...
      if (type === "receive") {
        const result = await dispatch(confirmReceived(commission._id));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
      <div className="w-full max-w-md rounded-xl border border-[#332b18] bg-[#111] shadow-2xl">
        ...
        <div className="p-6">
          ...
          {type === "receive" && (
            <p className="text-sm text-[#aaa]">
              Confirming this means you have received the commission payment
        ...
        <div className="p-6">
          ...
          {type === "paid" && (
            <p className="text-sm text-[#aaa]">
              This will mark the commission as fully paid. The payment
              method was already recorded when the listing owner sent the
              payment.
            </p>
          )}
        </div>
