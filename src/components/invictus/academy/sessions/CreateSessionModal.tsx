...
export default function CreateSessionModal({ open, onClose, onCreated }: Props) {
  ...
  const loadHosts = async () => {
    try {
      // যারা session host করতে পারবে — founder/manager
      const [foundersRes, managersRes] = await Promise.all([
        api.get("/users", { params: { role: "founder", limit: 50 } }),
        api.get("/users", { params: { role: "manager", limit: 50 } }),
      ]);

      setHosts([
        ...(foundersRes.data.data.data as HostUser[]),
        ...(managersRes.data.data.data as HostUser[]),
      ]);
    } catch (err) {
      console.log(err);
    }
